...
class FileTransferClient:
    def __init__(self, host, port, config):
        ...
        self.downloads_dir = config['CLIENT']['DOWNLOAD_DIR']
        ...
        logging.info(f"Download directory set to: {os.path.abspath(self.downloads_dir)}")
                ...
                if not self.session_id:
                    ...
                    if command.lower() == 'logout':
                        ...
                        if len(args) == 1:
                            ...
                            logging.warning("Usage: DOWNLOAD_SHARED<SEP>filename")
                    elif command.lower() == self.config['COMMANDS']['MAKE_PUBLIC_ADMIN'].lower():
                        if self.user_role == 'admin':
                            if len(args) == 1:
                                self.make_public_admin(args[0])
                            else:
                                logging.warning("Usage: MAKE_PUBLIC_ADMIN<SEP>filename")
                        else:
    ...
    def handle_upload(self, file_path, private):
        ...
        if response == self.config['RESPONSES']['READY_FOR_DATA']:
            with open(file_path, "rb") as f:
                with tqdm.tqdm(total=file_size, unit="B", unit_scale=True, unit_divisor=1024, desc=file_name) as progress:
                    while True:
    ...
    def handle_download_shared(self, file_name):
        ...
        command = f"{self.config['COMMANDS']['DOWNLOAD_SHARED']}{self.separator}{self.session_id}{self.separator}{file_name}"
        self.secure_socket.sendall(command.encode())
        self.receive_file(file_name)

    def receive_file(self, file_name):
        response_data = b''
        while self.separator.encode() not in response_data:
            chunk = self.secure_socket.recv(self.buffer_size)
            if not chunk:
                break
            response_data += chunk
        ...
        if status == self.config['RESPONSES']['DOWNLOAD_READY']:
            ...
            try:
                ...
                if final_response == self.config['RESPONSES']['DOWNLOAD_COMPLETE']:
                    logging.info(f"Successfully downloaded {file_name}.")
                else:
                    logging.warning(f"Download of {file_name} may be incomplete or corrupted. Server response: {final_response}")
            except (socket.timeout, ssl.SSLError) as e:
                logging.warning(f"Connection timed out or SSL error while waiting for final download response: {e}")
                logging.warning(f"Download of {file_name} may be incomplete or corrupted.")

        elif status == self.config['RESPONSES']['FILE_NOT_FOUND']:
            logging.error(f"File not found on server.")
        else:
    ...
    def make_public_admin(self, file_path):
        if self.user_role != 'admin':
            logging.warning("Permission denied. You must be an admin to use this command.")
            return

        command = f"{self.config['COMMANDS']['MAKE_PUBLIC_ADMIN']}{self.separator}{self.session_id}{self.separator}{file_path}"
        self.secure_socket.sendall(command.encode())
